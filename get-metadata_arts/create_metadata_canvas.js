...
registerFont(
    Path.resolve(__dirname, "assets", "fonts", "Poppins", "Poppins-Bold.ttf"),
    { family: "Poppins" }
);
registerFont(
    Path.resolve(
...
module.exports = async function CreateMetacontentCanvas({
    ...
    switch (type) {
        case "anime":
            content = await getPool(`SELECT * FROM anime WHERE slug="${slug}"`);
            break;
        case "manga":
            content = await getPool(`SELECT * FROM manga WHERE slug="${slug}"`);
            break;
        default:
            break;
    }

    if (!content) throw new Error("Data bulunamadı");
    content = content[0];
